...
void scan_video_option (const char *arg)
{
    ...
    if (!strcmp(arg, "ntsc_no_scanline"))
    {
        enable_ntsc = 1;
        ntsc_enable_scanlines = 0;
        save_pref_int("ntsc_enable_scanlines", 0);
    }

    if (!strcmp(arg, "ntsc_scanline"))
    {
        enable_ntsc = 1;
        ntsc_enable_scanlines = 1;
        save_pref_int("ntsc_enable_scanlines", 1);
    }
...
//           " -scanline       Interleaved scanline mode\n"
           ...
           " -ntsc_scanline  Enable scanlines in NTSC mode (default)\n"
           " -ntsc_no_scanline  Disable scanlines in NTSC mode\n"
		   ...
		   " -timeroffset SECONDS\n"
		   "                 Delay race timer start by SECONDS\n"
...
void cfg_parseargs (int argc, const char **argv)
{
  ...
  for (i = 1; i < argc; i++) {
    ...
    if (txt[0] == '-') {
      ...
      if (!strcmp(txt, "-timeroffset")) {
          if (1 != (argc - 1)) {
              i++;
              stopwatch_set_offset(atof(argv[i]));
          }
          else
          {
              printf("-timeroffset requires a numeric value in seconds.\n");
          }
      }
      ...
      // FIXME: store these in a pref file
      if (!strcmp(txt, "-joy0")) {
          if (argc<=(++i)) break;
...
void load_config (void)
{
    ...
    ntsc_enable_scanlines = pref_int("ntsc_enable_scanlines", 1);
}
